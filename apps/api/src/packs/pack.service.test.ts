...
describe('PackService', () => {
  ...
  it('passes founderIdea from niche mvpConcept JSON into pack prompt context', async () => {
    const { prisma } = makePrisma();

    const founderIdeaText = 'A complete platform for veterinary clinics in Southeast Asia to manage patient records, telemedicine, and inventory tracking — replacing pen-and-paper with an affordable mobile-first SaaS.';

    const founderMeta = JSON.stringify({
      intakeMode: 'founder_idea',
      founderIdea: founderIdeaText,
      executionMode: 'both',
      evidenceMode: 'starter_hypothesis',
      outputLanguage: 'ru',
      targetMarket: 'Southeast Asia',
      targetAudience: 'Veterinary clinics',
      productFormat: 'mobile_app',
    });

    // Override the niche.findFirst mock to return a founder-idea niche
    const originalFindFirst = prisma.niche.findFirst as ReturnType<typeof vi.fn>;
    originalFindFirst.mockResolvedValueOnce({
      id: 'n1', workspaceId: 'w1', projectId: 'p1',
      title: 'VetClinic Platform',
      oneLiner: founderIdeaText.slice(0, 200),
      problem: founderIdeaText,
      targetAudience: 'Veterinary clinics',
      whyNow: '',
      useCases: [],
      competitors: [],
      ...
      recommendedProductFormat: 'mobile_app',
      riskLevel: 'medium',
      scores: [{ totalScore: 50, confidenceValue: 0.25, confidenceLevel: 'low', explanation: 'starter', breakdown: [] }],
    });
    ...
    await svc.generate('w1', 'n1', { depth: 'build_ready', vertical: 'b2b_saas', useLlm: true });

    // The user prompt should include the founderIdea text
    const userMessage = routerRun.mock.calls[0]![0].messages[1].content as string;
    expect(userMessage).toContain('veterinary');
    expect(userMessage).toContain('Southeast Asia');
    expect(userMessage).toContain('Execution mode: both');
    expect(userMessage).not.toContain('"intakeMode"');
    expect(userMessage).not.toContain('"founderIdea"');
  });
});
