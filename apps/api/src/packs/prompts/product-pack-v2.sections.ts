...
export const PRODUCT_PACK_V2_SECTIONS: ProductPackV2SectionDefinition[] = [
  ...
  },
  {
    ...
    mustInclude: [
      ...
      'B. BCG Scorecard: a structured, scored entry per dimension (market growth, urgency of problem, buyer/user willingness to pay, competitive gap, differentiation, distribution access, retention/switching cost, monetization strength, defensibility/moat, evidence confidence, venture scale potential, execution feasibility) — 0-10 score, why-this-score, what-would-raise-it, and what-evidence-is-needed as separate structured fields, not prose or a markdown table',
      ...
      'E. Unicorn-grade Upside Path: what category/market expansion, platform/ecosystem move, data/network/workflow moat, distribution advantage, pricing/ACV/LTV path, and product-surface expansion would need to become true — using "could become", "requires proof", "depends on", "not proven yet"; never a factual unicorn claim',
      'F. Before / After Upgrade Table: a structured, scored entry per dimension — current score, weakness, upgrade move, target score after upgrades, why score improves — covering market growth, competitive position, distribution, retention, monetization, moat, evidence confidence, venture scale, execution feasibility; the improved score is explicitly a strategic target, not a proven fact',
      ...
      'this document is generated as structured JSON fields (see the BCG step contract), not as markdown prose — the backend renders the final markdown/tables from those fields',
    ],
  },
  {
  ...
  },
  {
    ...
    mustExplain: [
      ...
      'fields per entity',
      'relationships',
      'ownership / workspace or project scoping',
      'lifecycle states',
      'source of truth',
      'sensitivity',
    ...
    mustInclude: [
      'this document\'s sections[] is the full, readable specification — write one section per core entity (or a tight group of closely related entities), not a single summary section that only points at "the data model above"; the same entities named in this step\'s structured dataModel field belong here too, written out as prose a cold reader can act on without opening any other JSON',
      'for every entity: its purpose (why it exists, what real thing in this specific product it represents), its key fields with types and short descriptions, its relationships to other entities (has_many/belongs_to/etc, not just "related to X"), its lifecycle states if it has any (draft/active/archived/etc, or "N/A" if truly stateless — do not omit the question)',
      'ownership/scoping: state explicitly whether each entity is scoped to a user, a workspace, a project, or is global — this determines every access-control and multi-tenancy decision downstream',
      'source of truth: for any entity whose data could plausibly be duplicated or cached elsewhere (denormalized fields, external API mirrors, computed aggregates), state which system/table is authoritative',
      'privacy/sensitivity: label each field or entity as public, internal, personal, sensitive, or regulated — generic "some fields may be sensitive" is not acceptable, name which fields specifically and why',
      'audit/logging: state which entities need change history, who-did-what logging, or compliance audit trails, and which explicitly do not (with a one-line reason)',
      'at least one concrete example record or a short pseudo-schema per major entity (e.g. a realistic JSON-shaped example with plausible values for THIS product, not "id, name, createdAt")',
      'delete/export considerations (what happens on user/account deletion, what must be exportable)',
      'what must NOT be generic: entity names, fields, and relationships must be specific to this product\'s actual domain objects — a data model that would be identical for any unrelated SaaS product (e.g. only User/Workspace/Item/Account with generic CRUD fields) fails this section regardless of length',
    ],
  },
  {
    ...
    mustInclude: [
      'this document\'s sections[] is the full, readable specification — write one section per endpoint group (or a small number of closely related endpoints), not a single summary section that only points at "the API contracts above"; the same endpoints named in this step\'s structured apiContracts field belong here too, written out as prose a cold reader can act on without opening any other JSON',
      'every endpoint must be specific to what THIS product actually does — its path, method, what it is responsible for, its request shape, its response shape, and what side effects it triggers (records created, jobs queued, notifications sent)',
      'distinguish internal APIs (used only by this product\'s own frontend) from external/third-party integrations (payment, LLM, analytics, storage, messaging, etc.) this product genuinely needs, and explain what each integration is for',
      'async jobs/webhooks: if any action is not synchronous (background processing, scheduled jobs, incoming webhooks from a third party), name them and what triggers/consumes them; if genuinely none are needed, say so explicitly rather than omitting the question',
      'auth/permissions: state which endpoints require authentication, which roles/permissions gate which actions, and how unauthorized access is handled',
      'error cases: name the realistic failure modes for this product\'s core flows (not just a generic 400/401/500 list) — what happens on invalid input, conflicting state, rate limiting, or an upstream integration failure',
      'rate limits or reliability notes where relevant to this product (e.g. abuse-prone endpoints, expensive AI calls, third-party quotas) — omit only with a one-line reason if truly not applicable',
      'data contracts: the request/response shapes must be concrete (field names and types), not "returns the relevant data"',
      'frontend/backend interaction map: describe how the frontend actually uses these endpoints together for at least the product\'s core user flow(s), not just a flat endpoint list',
      'acceptance criteria specific to these endpoints (what "done" means for each), not generic REST best-practice boilerplate',
      'reject generic, product-agnostic endpoints (e.g. GET /users, POST /users, GET /items, or CRUD-only endpoints with no product-specific behavior) unless the product genuinely requires exactly that operation — if it does, explain why it is not more specific',
    ],
  },
  {
    key: 'ai_agent_pack',
