...
/**
 * Remote MCP server: Phase A1 read tools + Phase B operator write/execute
 * tools. PermissionsService/AuditService are provided globally; JwtService
 * comes from AuthModule (same secret as SignalKit login — MCP tokens are
 * distinguished by an `aud: 'mcp'` claim, never by a separate signer).
 */
@Module({
  imports: [
    AuthModule,
